...
There exists another resource, SemLink that provides a mapping between the labels from FN and PB. However, based on our
analysis of this resource (presented in Section \ref{sec:semlink}) the mapping has a very small coverage, is ambiguous and noisy in some cases. This limits our ability to tap into this resource.
...
Here we show the output parse on an example sentence upon applying SEMAFOR, which is a frame semantic parsing system based on FrameNet. We will focus on the parse errors that are a result of the gaps in FN annotations.
\begin{quote}
